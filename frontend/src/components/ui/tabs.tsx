...
const tabsListVariants = cva(
  "group/tabs-list inline-flex w-fit items-center justify-center text-muted-foreground group-data-[orientation=vertical]/tabs:h-fit group-data-[orientation=vertical]/tabs:flex-col",
  {
    variants: {
      variant: {
        // Capsule strip: hairline ring, roomy ends, the selected tab riding
        // inside it as its own pill.
        default:
  ...
  return (
    <TabsPrimitive.Tab
      ...
      className={cn(
        ...
        // Selected pill is the trigger's own background — the label's gradient
        // rides on the inner span, since one element gets one background.
        "data-active:bg-tab-active data-active:text-foreground data-active:shadow-[inset_0_0_0_1px_var(--tab-active-ring)] group-data-[variant=line]/tabs-list:bg-transparent group-data-[variant=line]/tabs-list:shadow-none",
        "after:absolute after:bg-foreground after:opacity-0 after:transition-opacity group-data-[orientation=horizontal]/tabs:after:inset-x-0 group-data-[orientation=horizontal]/tabs:after:bottom-[-5px] group-data-[orientation=horizontal]/tabs:after:h-0.5 group-data-[orientation=vertical]/tabs:after:inset-y-0 group-data-[orientation=vertical]/tabs:after:-right-1 group-data-[orientation=vertical]/tabs:after:w-0.5 group-data-[variant=line]/tabs-list:data-active:after:opacity-100",
        className
      )}
      {...props}
    >
      {/* Any icon keeps a solid colour: transparent text would take the glyph
          with it, since lucide strokes paint with currentColor. */}
      <span className="inline-flex items-center gap-1.5 dark:group-data-active/tab:bg-[image:var(--brand-gradient-text)] dark:group-data-active/tab:bg-clip-text dark:group-data-active/tab:text-transparent dark:group-data-active/tab:[&_svg]:text-foreground">
        {children}
      </span>
    </TabsPrimitive.Tab>
  )
